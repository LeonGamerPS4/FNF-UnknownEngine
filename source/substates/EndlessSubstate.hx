...
class EndlessSubstate extends MusicBeatSubstate
{
        ...
        if (selectable && !goingBack)
        {
			...
			*/
            ...
            if (controls.BACK)
            {
                ...
                {
                    closeSubState();
                    states.EndlessState.substated = false;
                });
            }
            ...
            if (controls.ACCEPT)
            {
				switch (optionShit[curSelected])
				{
					case 'play':
						...
						new FlxTimer().start(0.6, function(tmr:FlxTimer)
						{
							states.EndlessState.no = true;
							closeSubState();
							openSubState(new ChartSubstate());
						});
				}
            }
        }
        ...
        menuItems.forEach(function(spr:FlxSprite)
            {
                ...
                {
                    spr.scale.set(FlxMath.lerp(spr.scale.x, 0, camLerp/(ClientPrefs.data.framerate/60)), FlxMath.lerp(spr.scale.y, 0, 0.4/(ClientPrefs.data.framerate)));
                    spr.x = FlxMath.lerp(spr.x, 1500, camLerp/(ClientPrefs.data.framerate));
                }
            });
    }
