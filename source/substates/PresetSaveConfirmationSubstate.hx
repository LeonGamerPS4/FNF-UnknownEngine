...
class PresetSaveConfirmationSubstate extends MusicBeatSubstate
{
	#if desktop
	...
	public function new()
	{
		...
		resultName = new FlxText(FlxG.width * 0.7, 5, 1280, PresetSaveSubstate.nameResult, 96);
	...
	override function update(elapsed:Float)
	{
		...
		if (selectable && !goingBack)
		{
			if (controls.BACK)
			{
				...
				FlxG.state.openSubState(new PresetSaveSubstate());
			}

			if (controls.ACCEPT && canOK)
			{
				switch (PresetSaveSubstate.coming)
				{
					case "Modifiers":
						backend.ModifierPrefs.savePreset(eggText);
					/*
					case "Marathon":
				...
				new FlxTimer().start(0.5, function(tmr:FlxTimer)
				{
					...
					{
						case "Modifiers":
							FlxG.state.openSubState(new PresetSubstate());
						/*
						case "Marathon":
							FlxG.state.openSubState(new MarathonSubstate());
						case "Survival":
							FlxG.state.openSubState(new SurvivalSubstate());
						*/
					}
				});
			}
