...
class PresetSubstate extends MusicBeatSubstate
{
    ...
    public function new()
    {
        ...
        #if desktop
        presets = FileSystem.readDirectory('assets/presets/modifiers');
        presets.remove('current.json');
        ...
        if (selectable && !goingBack)
        {
            ...
            if (controls.ACCEPT)
            {
                switch (optionShit[curSelected])
                {
                    ...
                    case 'save':
                        goingBack = true;
                        PresetSaveSubstate.coming = "Modifiers";
                        ...
                        new FlxTimer().start(0.6, function(tmr:FlxTimer)
                        {
                            openSubState(new PresetSaveSubstate());
                            closeSubState();
                        });
                    case 'load':
                        goingBack = true;
                        PresetLoadSubstate.coming = "Modifiers";
                        ...
                        new FlxTimer().start(0.6, function(tmr:FlxTimer)
                        {
                            openSubState(new PresetLoadSubstate());
                            closeSubState();
                        });
                }
            }
